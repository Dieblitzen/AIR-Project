...
class PixorModel(object):
    ...
    def __init__(self): 
        ...
        self.y_class = tf.placeholder(tf.float32, shape=(None, TILE_SIZE, TILE_SIZE, 1), name='y_class')
        ...
        self.output_class = tf.layers.conv2d(inputs=header4, filters=1, kernel_size=3, padding='same', name='output_class')
    ...
    def get_loss(self):
        ...
        decoded_output = visualize_data.tf_pixor_to_corners(self.output_box)
        print('decoded_output.shape', decoded_output.shape) # (?, 224, 224, 4, 2)
        decoded_labels = visualize_data.tf_pixor_to_corners(self.y_box)
        decode_loss = 100 * decode_smooth_L1(box_labels=decoded_labels, box_preds=decoded_output, class_labels=self.y_class)
        
        box_loss = smooth_L1_loss
        pixor_loss = class_loss + box_loss
        decode_pixor_loss = class_loss + decode_loss

        return box_loss, pixor_loss, decode_loss, decode_pixor_loss
